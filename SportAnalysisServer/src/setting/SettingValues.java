...
//	public static final int SAME_COATCHING_ID_COUNT = 3;
	...
	/**
	 * コーチングスコア閾値
	 * この値以下の前パラメータがこのスコア以下の場合コーチングを行わない
	 */
	public static double COACH_SCORE_THRESHOLD = 100;

	/**
	 * コーチング頻度
	 * コーチモードでコーチングを行う頻度（何回に１回コーチングを行うか）
	 */
	public static int COACH_FREQ = 5;

	/**
	 * コーチング実施
	 * コーチモードでコーチングを実施するかどうかのスイッチ
	 * （1:実施 0:実施しない）
	 */
	...
	/**
	 * 状態推定頻度
	 * 状態推定モードでコーチングを行う頻度（何回に１回コーチングを行うか）
	 */
...
//	public static double BEST_EURZ_DIFF_AVE = 20.0;ストレート
	...
	public static String get_str_para(){
		...
		sb.append(",");
		sb.append(SettingValues.COACH_ONOFF);
		sb.append(",");
		sb.append(SettingValues.THRESHOLD_VAL);
		sb.append(",");
		sb.append(SettingValues.COACH_FREQ);
		sb.append(",");
		sb.append(SettingValues.COACH_SCORE_THRESHOLD);
		sb.append(",");
		sb.append(SettingValues.MAX_PITCH);
		sb.append(",");
		sb.append(SettingValues.MAX_DIFF_AVEZ);
		sb.append(",");
		sb.append(SettingValues.MAX_AVEX);
		sb.append(",");
		sb.append(SettingValues.MIN_PITCH);
		sb.append(",");
		sb.append(SettingValues.MIN_DIFF_AVEZ);
		sb.append(",");
		sb.append(SettingValues.MIN_AVEX);
		sb.append(",");
		sb.append(SettingValues.BEST_CYCLE);
		sb.append(",");
		sb.append(SettingValues.BEST_EURZ_DIFF_AVE);
		sb.append(",");
		sb.append(SettingValues.BEST_EURX_AVE);
		return sb.toString();
