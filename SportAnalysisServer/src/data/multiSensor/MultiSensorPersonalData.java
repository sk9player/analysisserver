...
public class MultiSensorPersonalData extends PersonalData{
	 ...
	 * センサ運動パラメータ
	 */
	public MultiSensorActionFeature features;

	/**
	 * 比較対象の教師センサ運動パラメータ
	 */
	public MultiSensorActionFeature teacher_features;
	...
	private void init(int sensor_num){
		...
		features = new MultiSensorActionFeature(sensor_num);
		teacher_features = new MultiSensorActionFeature(sensor_num);
	}
	...
	public double[][][] get_sensor_datas() {
		double[][][] sensor_datas= new double[sensor_num][SensorActionData.DATA_KIND][SettingValues.ANALYSIS_SENSOR_DATA_NUM * SettingValues.ANALYSIS_SENSOR_DATA_MUL_NUM];
		
		for(int i = 0;i<sensor_num;i++){
			sensor_datas[i][SensorActionData.EULX] = get_data_array(i,SensorActionData.EULX);
			sensor_datas[i][SensorActionData.EULY] = get_data_array(i,SensorActionData.EULY);
			sensor_datas[i][SensorActionData.EULZ] = get_eurz_array(i);
			sensor_datas[i][SensorActionData.WLINZ] = get_data_array(i,SensorActionData.WLINZ);
	 ...
	 * @return
	 */
	private double[] get_data_array(int sensor_id,int data_kind){
		...
		for (int i = 0; i < SettingValues.ANALYSIS_SENSOR_DATA_NUM;) {
			...
			if (late_commu_data_num_index == 0) {
				...
				SensorActionData act_data=multi_act_data_normalized_array[sensor_id][late_commu_num_index];
				if(act_data==null){
					return data;
				}
				
				late_commu_data_num_index = act_data
						.getMax_index() - 1;
				// 時系列波形の間の時間差を取得
				double diff_time=pre_time_data-act_data
						.getTime()[late_commu_data_num_index];
				
				int last_index=i-1;
				double last_data = act_data
						.getDataUsingID(data_kind)[late_commu_data_num_index];
				// 時系列波形の間を補間
